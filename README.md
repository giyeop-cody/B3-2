...
* **최단 경로 탐색 (`PATH`)**: Git의 실제 커밋 포인터는 자식 커밋에서 부모 커밋으로만 향하는 단방향 화살표입니다. 이 단방향 화살표만 그대로 따라가면, `feature` 브랜치 커밋에서 출발해 공통 부모까지는 올라갈 수 있지만, 거기서 `main` 브랜치의 자식 커밋으로는 내려갈 수가 없어서 서로 다른 브랜치 간의 경로를 찾을 수 없는 문제가 생깁니다. 그래서 `PATH` 명령어 연산을 할 때만큼은 두 커밋이 이어져 있는지 확인하기 위해 간선을 양방향(무방향)으로 재정의하여 탐색합니다. 탐색 알고리즘으로는 BFS(너비 우선 탐색)를 사용했는데, 커밋 간의 거리가 모두 1로 동일하기 때문에 레벨 단위로 퍼져나가는 BFS가 가장 짧은 간선 수의 최단 경로를 무조건 보장하기 때문입니다. 다중 최단 경로 발생 시 문자열 표현(`hash1->hash2->...`) 기준으로 사전순이 가장 작은 경로를 채택합니다.
...
*   **Q3. PATH 에서 최단 경로를 찾기 위해 어떤 알고리즘(예: BFS)을 선택했고, 간선을 무방향으로 정의한 이유를 설명할 수 있는가?**
    *   **답변**: Git의 실제 커밋 포인터는 자식 커밋에서 부모 커밋으로만 향하는 단방향 화살표입니다. 이 단방향 화살표만 그대로 따라가면, `feature` 브랜치 커밋에서 출발해 공통 부모까지는 올라갈 수 있지만, 거기서 `main` 브랜치의 자식 커밋으로는 내려갈 수가 없어서 서로 다른 브랜치 간의 경로를 찾을 수 없는 문제가 생깁니다. 그래서 `PATH` 명령어 연산을 할 때만큼은 두 커밋이 이어져 있는지 확인하기 위해 간선을 양방향(무방향)으로 재정의하여 탐색합니다. 탐색 알고리즘으로는 BFS(너비 우선 탐색)를 사용했는데, 커밋 간의 거리가 모두 1로 동일하기 때문에 레벨 단위로 퍼져나가는 BFS가 가장 짧은 간선 수의 최단 경로를 무조건 보장하기 때문입니다.
...
본 프로젝트의 심층 이해와 동료 평가(Peer Review) 자료는 전용 브랜치인 [**`peer-evaluation` 브랜치**](https://github.com/giyeop-cody/B3-2/tree/peer-evaluation)에서 확인하실 수 있습니다.

*   **[peer-evaluation 브랜치 바로가기](https://github.com/giyeop-cody/B3-2/tree/peer-evaluation)**:
    *   `PEER_EVALUATION_GUIDE.md`: 동료 평가자 이해도 수준별(초심자/숙련자) 채점 가이드.
    *   `dag_visualizer.html`: 대화형 DAG, Kahn's Algo, BFS, 역색인 애니메이션 시뮬레이터.
    *   `evaluation_scenario_1~4.md`: 전수 평가 시나리오 마크다운 4종.
    *   `MINI_GIT_COMPREHENSIVE_REPORT.md`: 과제 심층 종합 보고서.
*   **[패키지별 기술 문서]**:
